...
Design philosphy never have fatal errrors - you want to maximise node availability (fail-live?) If there is an error shrug it off and try something else


Despite the benefits of a decentralised approach, the majority of popular P2P networks include structured elements such as lookup tables, super-peers\cite{super_peers} or DHTs(Distributed Hash Tables). These are introduced primarily to improve network performance by reducing message complexity. However, this then reintroduces some of the primary pitfalls of the client-server model. Therefore, the implementation of an entirely unstructured P2P network service designed for information availability and fault tolerance can have significant benefits. (this last sentence needs improving)
